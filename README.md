...
public class Example
{
    ...
    {
        // Initialize ADBueno
        var adb = new AdbClient(); // Or use specific device with AdbClient.Connect("your_device_serial")

        // Example: Detect devices
        var devices = await adb.DetectDevices();
        if (devices.Count > 0)
        {
            ...
            string output = await adb.RunShellCommand(firstDevice, "ls /sdcard/");
            Console.WriteLine($"\nOutput from 'ls /sdcard/' on {firstDevice}:");
            Console.WriteLine(output);

            // Example: Check if boot is completed
            bool bootCompleted = await adb.CheckBootCompleted(firstDevice);
            Console.WriteLine($"\nBoot completed on {firstDevice}: {bootCompleted}");
        }
        else
        {
            Console.WriteLine("No ADB devices found.");
        }
    }
}
...
Please make sure to read our CONTRIBUTING.md for details on our code of conduct, and the process for submitting pull requests to us.
